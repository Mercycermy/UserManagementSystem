...
class ChangePassword(View):
    def get(self, request):
        if not request.user.is_authenticated:
            messages.add_message(request, messages.ERROR, "You aren't logged in.")
            return redirect('/')
    ...
    def post(self, request):
        ...
        if len(newpass) < 8:
            messages.add_message(request, messages.ERROR, "New password can't be less than 8 characters.")
            error += 1
